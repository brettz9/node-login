...
module.exports = function (app, config) {
  ...
  */
  app.post('/lost-password', function (req, res) {
    ...
    AM.generatePasswordKey(email, req.ip, (e, account) => {
      if (e) {
        res.status(400).send(e);
      } else {
        ED.dispatchResetPasswordLink(account, (_e, m) => {
          // TODO this callback takes a moment to return, add a loader to
          //   give user feedback
          if (!_e) {
            res.status(200).send('ok');
          } else {
            for (const k in _e) {
              if ({}.hasOwnProperty.call(_e, k)) {
